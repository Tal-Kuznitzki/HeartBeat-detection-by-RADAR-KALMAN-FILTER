...
%%%%
%running seperatly on 3 different scenarios.
...
for indx = 1:length(IDrange)
    ...
    for sz = 1:length(scenarios)
        ...
        %fprintf('corr between tfm_ecg_locsR and radar_hs_locsR: %s\n', num2str(corr(tfm_ecg_locsR,radar_hs_locsR(2:721))));
        fprintf('---- done\n');
    end
end
...
for sz = 1:length(scenarios)
    ...
    %fprintf('correlation between radar and ECG on scenario "%s" is: %s\n ', scenario,num2str(corr_radar_ecg));
