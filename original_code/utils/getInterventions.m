...
y = [st en];
locs = getTriggerlocs(intervention);
...
if strcmp(scenario,'Valsalva')
    ...
    % That means typically 6 edges, but it can happen that not all are
    % detected
    if length(locs) < 6
...
elseif strcmp(scenario,'Apnea')
    ...
    % breath
    locs_start = getTriggerlocs(intervention,2); % falling edge is start
    locs_end = getTriggerlocs(intervention,1); % rising edge is end
